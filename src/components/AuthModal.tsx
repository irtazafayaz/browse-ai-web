...
import { X, Mail, Lock, User, Eye, EyeOff, Sparkles, ArrowRight, LogOut } from 'lucide-react';
...
export default function AuthModal({ open, onClose }: Props) {
  ...
  if (user) {
    ...
    return (
      ...
      >
        <div
          className="w-full max-w-sm animate-fade-slide-up"
        ...
        >
          ...
          <div className="px-6 pb-6">
            <button
              ...
              className="w-full py-3 flex items-center justify-center gap-2 text-sm font-bold text-[#1A1A1A] transition-all hover:bg-[#E8E0D4] active:scale-[0.98]"
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try { setError(Object.values(JSON.parse(msg)).flat().join(' ')); }
      catch { setError(msg); }
    } finally {
      setLoading(false);
    }
              ...
              >
                {loading ? (
                  <span className="flex gap-1">
                    {[0,1,2].map(i => <span key={i} className="w-1.5 h-1.5 bg-white rounded-full animate-bounce" style={{ animationDelay: `${i*120}ms` }} />)}
                  </span>
                ) : (
            ...
            <form onSubmit={handleRegister} className="flex flex-col gap-3.5">
              ...
              >
                {loading ? (
                  ...
                  <><User size={13} /> Create account <ArrowRight size={13} /></>
                )}
              </button>
              <p className="text-center text-[11px] text-[#9B9B9B]">
                Already have one?{' '}
