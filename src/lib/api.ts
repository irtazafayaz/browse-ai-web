...
function parseDrfError(err: unknown, fallback: string): string {
  ...
  for (const val of Object.values(e)) {
    if (Array.isArray(val))
      val.forEach((v) => typeof v === "string" && msgs.push(v));
...
export async function login(
  ...
  if (!res.ok)
    throw new Error(
      parseDrfError(body, "Login failed. Check your credentials."),
    );
  const data = body as unknown as AuthResponse;
  if (!data?.access || !data?.refresh)
    throw new Error("Invalid response from server.");
...
export async function toggleBookmark(productId: string) {
  const res = await apiFetch(`/api/products/${productId}/bookmark/`, {
    method: "POST",
  });
...
export async function getEdits(): Promise<
  { label: string; imageUrl: string; tag: string }[]
> {
